...
import { SinglePlayerIcon, DoublePlayerIcon, QuadPlayerIcon, ClockIcon, InformationCircleIcon, UserIcon, XIcon } from '../ui/Icons';

const DeviceCard: React.FC<{ device: Device }> = ({ device }) => {
  const { devices, sessions, endSession, updateSession, prices } = useAppContext();
  ...
  useEffect(() => {
    ...
    if (session?.status === 'active' && session.timeMode === 'timed' && session.endTime) {
      // FIX: Use window.setInterval to explicitly use the browser's implementation which returns a number, resolving the TypeScript type conflict with Node.js's Timeout object.
  ...
  const handleEndSessionRequest = () => {
    ...
    switch (session.gameType) {
        case GameType.Single:
            pricePerHour = devicePrices.single;
            break;
        case GameType.Double:
            pricePerHour = devicePrices.double;
            break;
        case GameType.Quad:
            pricePerHour = devicePrices.quad;
            break;
        default:
            pricePerHour = 0;
    }
    ...
    const toastId = `confirm-end-session-${device.id}`;
  ...
  return (
    <>
      <div className={`relative p-4 rounded-lg border-2 shadow-sm transition-all duration-300 ${getStatusClasses()}`}>
        ...
        <h3 className="text-xl font-bold text-center mt-6">{device.name} <span className="text-sm font-normal bg-gray-200 dark:bg-gray-700 px-2 py-1 rounded">{device.type}</span></h3>
            ...
            {device.status === DeviceStatus.Busy && session && (
                <div className="w-full text-center space-y-2">
                    ...
                    <div className="flex justify-center items-center gap-4 text-lg font-semibold">
                       {
                          session.gameType === 'single' ? <SinglePlayerIcon className="h-7 w-7"/> : 
                          session.gameType === 'double' ? <DoublePlayerIcon className="h-7 w-7"/> : 
                          <QuadPlayerIcon className="h-7 w-7" />
                        }
